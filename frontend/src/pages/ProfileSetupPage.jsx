  ...
  return <div className={`profile-setup setup-flow${editing ? ' profile-edit' : ''}`}>
    ...
    <main className="profile-grid">
      <form className="profile-card" onSubmit={finish} noValidate><p className="setup-progress">{editing ? 'Your profile' : `Step ${step} of 3`}</p><div className="profile-title-row">{editing && <a className="profile-title-back" href="/home" onClick={navigate} aria-label="Back to home">&#8592;</a>}<h1>{editing ? 'Edit profile.' : step === 1 ? 'Your classes.' : step === 2 ? 'A little about you.' : 'Pick an avatar.'}</h1></div>{saveError && <p role="alert" className="error">{saveError}</p>}
        ...
        {(editing || step === 2) && <section><div className="field"><label htmlFor="year">Year (optional)</label><select id="year" value={profile.year} onChange={event => update('year', event.target.value)}><option value="">Select year</option>{['First', 'Second', 'Third', 'Fourth', 'Fifth+'].map(year => <option key={year}>{year}</option>)}</select></div><div className="field"><label htmlFor="bio">Bio (optional)</label><textarea id="bio" rows={4} maxLength={500} value={profile.bio} onChange={event => update('bio', event.target.value)} /></div>{editing && <label className="offline-discovery"><input type="checkbox" checked={Boolean(profile.offlineDiscoverable)} onChange={event => update('offlineDiscoverable', event.target.checked)} /><span><strong>Discover me when I’m offline</strong><small>People can send a study request while you are away. You’ll be notified when it becomes a match.</small></span></label>}</section>}
