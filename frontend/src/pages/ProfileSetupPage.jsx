...
const STUDY_HOURS = Array.from({ length: 17 }, (_, index) => index + 6);

function formatStudyTime(slot) {
  const day = DAYS[Math.floor(slot / 24)];
  const hour = slot % 24;
  return day + ' ' + (hour % 12 || 12) + ':00 ' + (hour < 12 ? 'AM' : 'PM');
}
...
export default function ProfileSetupPage({ profile: savedProfile, onProfileChange: saveProfile, onAvatarSelect, navigate, goTo, editing = false, step = 1, setupDraft, onSetupDraft }) {
  ...
  function addStudyTime() {
    const slot = Number(studyDay) * 24 + Number(studyHour);
  ...
  return <div className={`profile-setup setup-flow${editing ? ' profile-edit' : ''}`}>
    ...
    <main className="profile-grid">
      <form className="profile-card" onSubmit={finish} noValidate><p className="setup-progress">{editing ? 'Your profile' : `Step ${step} of 3`}</p><h1>{editing ? 'Edit profile.' : step === 1 ? 'Your classes.' : step === 2 ? 'A little about you.' : 'Pick an avatar.'}</h1>{saveError && <p role="alert" className="error">{saveError}</p>}
        ...
        {(editing || step === 2) && <section><div className="field"><label htmlFor="year">Year (optional)</label><select id="year" value={profile.year} onChange={event => update('year', event.target.value)}><option value="">Select year</option>{['First', 'Second', 'Third', 'Fourth', 'Fifth+'].map(year => <option key={year}>{year}</option>)}</select></div><div className="field"><label htmlFor="bio">Bio (optional)</label><textarea id="bio" rows={4} maxLength={500} value={profile.bio} onChange={event => update('bio', event.target.value)} /></div><div className="profile-study-times"><div className="profile-section-title"><h2 id="study-times-title">When do you like to study?</h2><span>Optional</span></div><p className="hint">Add the weekly hours you are generally available.</p><div className="profile-time-entry"><label><span className="sr-only">Day</span><select value={studyDay} onChange={event => setStudyDay(event.target.value)}>{DAYS.map((day, index) => <option key={day} value={index}>{day}</option>)}</select></label><label><span className="sr-only">Time</span><select value={studyHour} onChange={event => setStudyHour(event.target.value)}>{STUDY_HOURS.map(hour => <option key={hour} value={hour}>{(hour % 12 || 12) + ':00 ' + (hour < 12 ? 'AM' : 'PM')}</option>)}</select></label><button type="button" onClick={addStudyTime}>Add time +</button></div><ul className="profile-time-list" aria-label="Added study times">{(profile.studyTimes || []).map(slot => <li key={slot}>{formatStudyTime(slot)}<button type="button" aria-label={'Remove ' + formatStudyTime(slot)} onClick={() => update('studyTimes', profile.studyTimes.filter(value => value !== slot))}>×</button></li>)}</ul></div></section>}
