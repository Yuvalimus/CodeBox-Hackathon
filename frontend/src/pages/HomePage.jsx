...
export default function HomePage({ profile, navigate }) {
  ...
  return <div className="buddy-home">
    ...
    <main className="home-main">
      ...
      {session && <section className="home-session" role="status"><div className="eyebrow">TEST SESSION READY</div><h2>You’ve got a study plan.</h2><p><strong>Classes:</strong> {session.classes.join(', ')}</p><p><strong>Location:</strong> {session.location}</p><p>This is a local preview. Buddy discovery isn’t connected, and you aren’t visible to other students.</p><button className="home-secondary" onClick={() => { setSession(null); setSelected([]); setLocation(''); }}>End test session</button></section>}
