...
export default function HomePage({ profile, navigate, onLogout, loggingOut, logoutError, session, setSession, active = true, onPresenceError }) {
  ...
  const looking = Boolean(active && session && !session.testDeck);
  useEffect(() => {
    if (wasLooking.current && !looking) goOffline().catch(error => onPresenceError?.(error.message));
    wasLooking.current = looking;
  }, [looking, onPresenceError]);
  ...
  return <div className="buddy-home">
    <PresenceHeartbeat active={looking} onError={onPresenceError || (() => {})} />
