...
export function fromUser(user, local = {}) {
  return { ...user, pictureUrl: mediaUrl(user.pictureUrl), name: user.username || local.name, year: local.year || '', avatar: user.avatar || local.avatar || 'sage', photo: local.photo || null, classes: user.classes || [], studyTimes: user.studyTimes || [], major: user.major || '', bio: user.bio || '' };
}
export function profileBody(profile) {
  return { username: profile.name.trim(), classes: profile.classes, studying: (profile.studying || []).filter((course) => profile.classes.includes(course)), studyTimes: profile.studyTimes || [], major: profile.major, bio: profile.bio, avatar: profile.avatar || 'sage', ...(!profile.pictureUrl ? { pictureUrl: null } : {}) };
}
