    ...
    {apiError && <p className="error" role="alert">{apiError} <button disabled={busy} onClick={() => setReload(value => value + 1)}>Retry loading</button></p>}{loading && <p role="status">Loading online study buddies...</p>}<div className="discovery-layout"><div className="discovery-main">
      {!loading && candidate ? <>
        <article key={candidate.id} className={`candidate-card ${exitDirection ? `card-exit-${exitDirection}` : drag ? "card-dragging" : "card-enter"}`} style={{ transform: exitDirection ? undefined : `translateX(${Math.max(-130, Math.min(130, drag))}px) rotate(${drag / 35}deg)` }} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={() => { gesture.current = null; setDrag(0); }}>
          ...
          <div className="candidate-content"><h3>{candidate.name}</h3>{(candidate.major || candidate.year) && <p className="candidate-detail">{[candidate.major, candidate.year && `${candidate.year} year`].filter(Boolean).join(' · ')}</p>}<div className="candidate-common"><span>Classes you both chose for this session</span><ul>{candidate.classes.map((course) => <li key={course}>{course}</li>)}</ul></div>{candidate.comments && <p className="candidate-comments"><strong>Additional info:</strong> {candidate.comments}</p>}{candidate.bio && <p className="candidate-bio">{candidate.bio}</p>}{candidate.location && <p className="candidate-location">Study spot · {candidate.location}</p>}</div>
        </article>
