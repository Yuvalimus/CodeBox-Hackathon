...
export default function DiscoveryDeck({ session, onEnd, active = true }) {
  ...
  const decide = useCallback(async (direction) => {
    ...
    const [result] = await Promise.all([
      ...
      new Promise(resolve => setTimeout(resolve, window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : window.matchMedia('(hover: hover) and (pointer: fine)').matches ? 400 : 280)),
    ]);
    if (direction === 'right') {
  ...
  function pointerMove(event) {
    ...
    if (!start.axis && Math.max(Math.abs(dx), Math.abs(dy)) > 8) {
      if (Math.abs(dx) > Math.abs(dy)) start.axis = 'x';
      else if (Math.abs(dy) > Math.abs(dx) * 1.5) start.axis = 'y';
    }
