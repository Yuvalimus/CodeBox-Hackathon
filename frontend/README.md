...
API integration is intentionally unimplemented. Valid submissions show a preview notice and do not authenticate, create an account, verify email ownership, or persist credentials. The integration placeholder is in `src/components/auth/AuthForm.jsx` inside `submit`. Backend validation and the final password policy must be coordinated when connecting authentication.
